# -*- coding: utf-8 -*-
"""Testy przypomnień PL materiały (interwał 3 dni, jedno przypomnienie)."""
...
class TestPlReminderEmail(unittest.TestCase):
    def test_pl_reminder_contains_polish_text(self):
        contact = {
            "company_name_clean": "Budmat PL",
            "email_subject_sent": "Zapytanie ofertowe",
            "email_body_sent": "Dzień dobry,\n\nproszę o wycenę.\n\nZ poważaniem,\nMaksym Swinczak",
            "email_sent_at": "2026-07-01T10:00:00",
            "email_status": "sent",
            "email_target": "info@budmat.pl",
        }
...
class TestPlReminderTiming(unittest.TestCase):
    def test_interval_constants(self):
        self.assertEqual(MATERIALY_REMINDER_INTERVAL_DAYS, 3)
        self.assertEqual(MATERIALY_REMINDER_INTERVAL_HOURS, 72.0)
        self.assertEqual(MATERIALY_MAX_REMINDERS_PER_CONTACT, 1)

    def test_pending_after_3_days(self):
        contact = {
            "email_status": "sent",
            "email_target": "info@test.pl",
    ...
    def test_backfill_existing_replies(self):
        ...
        self.assertEqual(cache["contacts"]["https://x.pl"]["email_status"], "replied")
