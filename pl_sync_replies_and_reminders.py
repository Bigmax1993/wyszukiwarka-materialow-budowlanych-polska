# -*- coding: utf-8 -*-
"""
PL materiały: odczyt odpowiedzi Gmail (IMAP) + przypomnienia do firm bez odpowiedzi.
...
Domyślnie TRYB PODGLĄDU (bez wysyłki). Aby wysłać maile:
  python pl_sync_replies_and_reminders.py --send

Przykłady:
  python pl_sync_replies_and_reminders.py
  python pl_sync_replies_and_reminders.py --send
  python pl_sync_replies_and_reminders.py --send --min-days 3 --max-send 50
"""
...
from scraper_email_replies import (  # noqa: E402
    ...
    MATERIALY_MAX_REMINDERS_PER_CONTACT,
    MATERIALY_REMINDER_INTERVAL_DAYS,
    MATERIALY_REMINDER_INTERVAL_HOURS,
    ReplySyncConfig,
...
def pl_preset() -> dict:
    paths = campaign_output_paths(REPO_ROOT, "pl_materialy")
    return {
...
def setup_logging() -> logging.Logger:
    return setup_module_logging("pl_sync_replies_and_reminders")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="PL materiały: skan IMAP + przypomnienia (co 3 dni po braku odpowiedzi)"
    )
    parser.add_argument(
        "--send",
        action="store_true",
    ...
    parser.add_argument(
        "--min-days",
        type=float,
        default=MATERIALY_REMINDER_INTERVAL_DAYS,
        help="Min. dni od wysyłki pierwszego maila",
    )
    parser.add_argument(
        "--imap-days",
        type=int,
    ...
    for place_url, contact, target in candidates:
        ...
        if sent_count >= args.max_send:
            logger.info(
                "Osiągnięto limit --max-send=%s — reszta w następnym uruchomieniu.",
                args.max_send,
            )
            break
        ...
        ok, info = send_email_gmail(
            ...
            body,
            logger,
            mail_type=f"przypomnienie PL {rem_num}/{MATERIALY_MAX_REMINDERS_PER_CONTACT}",
