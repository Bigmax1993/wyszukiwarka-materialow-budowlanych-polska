...
E-mail UA (polski) przez yagmail/Gmail w tym module (send_email_pl_materialy).
...
_paths = campaign_output_paths(_campaign, "pl_materialy")
...
def apply_pl_run_config_extras(module, data: dict) -> None:
...
import pl_materialy_supplier_filter as _retail_store_builder_filter
...
# =========================
# KONFIGURATION – PL materiały budowlane (hurtownie / składy)
# =========================
...
REGION_CENTER_LAT = 52.0693
REGION_CENTER_LON = 19.4803
...
EMAIL_SUBJECT_TEMPLATE = "Zapytanie o dostawę materiałów budowlanych"
...
INQUIRY_REGION_PL = "Polska"
MATERIAL_CATEGORIES_PL = "cement, piasek, żwir, cegła, bloczek, armatura, styropian"
...
from pl_materialy_inquiry_email_pl import FIXED_MATERIAL_INQUIRY_PL, build_fixed_material_inquiry_pl
ENABLE_EMAIL_ATTACHMENT = False
PL_EMAIL_ALLOW_ATTACHMENTS = False
...
def get_email_attachments_pl_materialy(logger: logging.Logger | None = None) -> list[str]:
    """Kampania UA — zawsze bez załączników (tylko treść maila)."""
    _ = logger
    if ENABLE_EMAIL_ATTACHMENT or PL_EMAIL_ALLOW_ATTACHMENTS:
        logger and logger.warning(
            "PL materialy: załączniki wyłączone — ignoruję ENABLE_EMAIL_ATTACHMENT"
        )
    return []
...
def setup_logging() -> logging.Logger:
    ...
    return setup_script_logging("pl_materialy_scraper", LOG_FILE)
...
def row_to_excel_kontakte_columns(row: dict, email: str = "") -> dict:
    ...
    return {
        ...
        "Województwo": (row.get("bundesland") or row.get("discovery_bundesland") or "").strip(),
...
def enrich_row_with_claude_cleanup(row: dict, logger: logging.Logger, cache: dict) -> dict:
    ...
    if claude_result["bundesland"] not in PL_WOJEWODZTWA:
...
def build_bundesland_rows(rows):
    ...
    for row in rows:
        ...
        if row_name.lower() == "nieznana firma" and not row_url:
            continue
        row_state = (table.get("Województwo") or "").strip()
...
def sanitize_generated_email(subject: str, body: str, company_name: str):
    ...
    clean_body = ensure_inquiry_signature(clean_body)
    clean_body = strip_german_phones_from_text(clean_body)
    clean_body = strip_legacy_branding_preserve_layout(clean_body)
    clean_body = re.sub(r"\n{3,}", "\n\n", clean_body).strip()
    return clean_subject, clean_body


def sanitize_sender_name(sender_name: str) -> str:
    from pl_materialy_inquiry_email_pl import inquiry_company_name, inquiry_sender_name
...
def verify_company_on_website(
    ...
    *,
    ...
    if ENABLE_CLAUDE_PAGE_VERIFY:
        ...
        from pl_claude_page_verify import claude_verify_company_page
...
def is_germany_de_candidate(link: str, title: str = "", snippet: str = "") -> bool:
    ...
    if re.search(r"\b\d{5}\b", text):
        return True
    if any(x in text for x in PL_COUNTRY_HINTS):
        return True
...
def _assemble_inquiry_email_body(company_name: str, opening: str = "") -> str:
    ...
    return FIXED_MATERIAL_INQUIRY_PL.strip()
...
def generate_email_content(
    ...
    *,
    ...
    if ENABLE_CLAUDE_INQUIRY_EMAIL:
        ...
        generated = claude_generate_inquiry_email_pl(
            display_name,
            logger,
            cache,
...
def send_email_pl_materialy(
    ...
    logger.info("PL materialy: Cc=%s | załączniki: brak", ", ".join(cc) if cc else "(brak)")
    return send_smtp_email(
        to_email,
        subject,
        body,
        logger,
        mail_type="PL materialy",
        campaign="pl_materialy",
        cc=cc,
        attachment_paths=[],
    )
    ...
    *,
    ...
    for mail in jobs_to_send:
        ...
        if dry_run_email:
            ...
            ok, info = send_email_pl_materialy(target, subject, body, logger)
...
def run_scraper(
    ...
    **_deprecated_kwargs,
    ...
    rotate_mode = bool(rotate_wojewodztwo and discovery_mode != "emails_only")
    from pl_wojewodztwo_rotation import rotation_is_active

    if rotate_wojewodztwo and discovery_mode != "emails_only":
    ...
    if rotate_wojewodztwo and discovery_mode != "emails_only" and not rebuild_from_cache:
        ...
        if rotation_is_active():
            ...
            print(
                f"[ROTACJA] Discovery dla województwa: {rotation_land} "
                f"(1 województwo / cykl, min. nowych firm={MIN_CONTACTS_TARGET})"
            )
            ...
            print(
                ...
                f"{len(CAMPAIGN_ACTIVE_BUNDESLAENDER)} województw."
            )

    logger.info("=== START PL materiały budowlane — hurtownie / składy (Serper API) ===")
    start_scraper_runtime_clock()
    ...
    if (
        ...
        from pl_wojewodztwo_rotation import commit_rotation_after_run
...
def _run_smoke_tests() -> None:
    ...
    assert "Polska" in INQUIRY_REGION_PL
    assert "cement" in MATERIAL_CATEGORIES_PL
    assert "materiałów budowlanych" in FIXED_MATERIAL_INQUIRY_PL
    assert "cement" in FIXED_MATERIAL_INQUIRY_PL
    assert "materiałów budowlanych" in FIXED_EMAIL_SUBJECT_PL
    ...
    prompt = build_personalized_inquiry_email_prompt_pl(
        company_name="Hurtownia Budowlana Warszawa",
        website="https://budmat.pl",
        wojewodztwo="mazowieckie",
        materials="cement, piasek",
        page_snippet="Hurtownia materiałów budowlanych w Warszawie",
    )
    assert "polsk" in prompt.lower()
    assert "Hurtownia Budowlana Warszawa" in prompt
    cleaned = sanitize_email_body(FIXED_MATERIAL_INQUIRY_PL)
    assert "\n\n" in cleaned
    assert "Szanowni" in cleaned
    ...
    assert is_valid_retail_store_builder_contact(
        name="Skład budowlany Kraków",
        url="https://budmat-krakow.pl",
        email="info@budmat-krakow.pl",
        text="Sprzedaż cementu i piasku hurt. Katalog produktów.",
    )
    assert not is_valid_retail_store_builder_contact(
        url="https://www.olx.pl/ogloszenie",
        name="OLX ogłoszenie",
        text="Sprzedam używaną cegłę",
    )
    assert is_media_publisher_contact(
        url="https://www.news.pl/budownictwo",
        name="news.pl",
        email="redakcja@news.pl",
    )
    assert not is_loose_serper_discovery_candidate(
        url="https://www.news.pl/budownictwo",
        name="news.pl",
        text="Wiadomości budowlane",
    )
    from pl_wojewodztwo_keywords import (
        SERPER_DISCOVERY_BROAD_TERMS,
        SERPER_DISCOVERY_LANDKREIS_TERMS,
    ...
    from pl_materialy_supplier_filter import is_serper_only_pending_candidate

    assert is_serper_only_pending_candidate(
        name="Hurtownia Budowlana Kraków",
        url="https://budmat-krakow.pl",
        text="materiały budowlane hurt cement piasek",
    )
    assert not is_serper_only_pending_candidate(
        name="Studio designu", url="https://design.pl", text="wykończenia wnętrz"
    )
    assert is_unsuitable_inquiry_email("privacy@firma.pl")
    best, sc = pick_best_email_for_inquiry(
        ["datenschutz@shop.pl", "info@budmat.pl"],
        "https://www.budmat.pl/",
    )
    assert best == "info@budmat.pl" and sc >= MIN_EMAIL_SCORE_FOR_SEND
    assert is_row_eligible_for_excel_export(
        {
            "nazwa": "Hurtownia Budowlana Warszawa Sp. z o.o.",
            "url": "https://budmat.pl",
            "email_target": "",
            "retail_verified": True,
            "is_gu": True,
            "gu_marker": "materiały budowlane",
            "verification_reason": "claude_material_supplier",
            "page_snippet": "Hurtownia materiałów budowlanych cement piasek",
            "retail_chains_found": "cement",
        }
    )
    assert not is_row_eligible_for_excel_export(
        {
            "nazwa": "Wykończenia wnętrz",
            "url": "https://design.pl",
            "email_target": "",
            "retail_verified": True,
    ...
    assert "wiadomości" in SERPER_NEGATIVE_TERMS
    ...
    assert not is_waf_blocked(html="<html><body>Hurtownia budowlana Warszawa</body></html>")
...
if __name__ == "__main__":
    ...
    if "--test" in sys.argv:
        ...
        if "--wojewodztwo" in sys.argv or "--bundesland" in sys.argv:
            i = sys.argv.index("--wojewodztwo") if "--wojewodztwo" in sys.argv else sys.argv.index("--bundesland")
            if i + 1 < len(sys.argv):
                bl = sys.argv[i + 1].split(",")
                configure_campaign_bundeslaender(sys.modules[__name__], bl)
                print(f"[TRYB] Aktywne województwa: {', '.join(CAMPAIGN_ACTIVE_BUNDESLAENDER)}")
        if "--rotation-status" in sys.argv:
            from pl_wojewodztwo_rotation import format_rotation_status

            print(format_rotation_status(OUTPUT_DIR))
            raise SystemExit(0)
        rotate_bl = "--rotate-wojewodztwo" in sys.argv
        if rotate_bl:
            extra_kw["rotate_wojewodztwo"] = True
        ...
        if rc_path:
            ...
            try:
                with open(Path(rc_path), encoding="utf-8") as _rcf:
                    apply_pl_run_config_extras(mod, json.load(_rcf))
            except Exception:
                pass
            launch_kw = run_scraper_launch_kwargs(mod)
