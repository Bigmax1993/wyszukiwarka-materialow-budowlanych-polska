...
<#
Uruchamia pelny pipeline PL na GitHub Actions (recznie, krok po kroku).

  powershell -ExecutionPolicy Bypass -File scripts\run_full_pipeline_gha.ps1

Opcje:
  -SkipDiscovery   pomin discovery
  -SkipBackfill    pomin backfill (sync -> prep -> send po gotowym thu)
  -ForceResend     ponowna wysylka (--force-resend na obu partiach)
  -FullDiscovery   uruchom 5 etapow discovery (mon..fri) po kolei
  -StrictDiscovery przy timeout/failure discovery przerwij
#>
param(
...
$Repo = "Bigmax1993/wyszukiwarka-materialow-budowlanych-polska"
$DiscoveryWorkflow = "PL discovery"
$DiscoveryArtifacts = @("pl-materialy-wyniki-pi")
...
function Test-RunHasDiscoveryArtifact {
    param([string]$RunId, [string[]]$AlsoCheckRunIds = @())
...
function Wait-GhaRun {
    param([string]$Name, [string]$RunId, [switch]$ContinueOnDiscoveryArtifact)
...
function Invoke-GhaWorkflow {
    param([string]$Name, [hashtable]$Fields = @{}, [switch]$ContinueOnDiscoveryArtifact)
    Write-Host "`n=== $Name ===" -ForegroundColor Cyan
    if ($Fields.Count -gt 0) {
        $wfArgs = @()
        foreach ($k in $Fields.Keys) { $wfArgs += "-f"; $wfArgs += "${k}=$($Fields[$k])" }
    ...
    if ($ContinueOnDiscoveryArtifact) { $script:DiscoveryRunIds += $runId }
    return $runId
}
...
if ($DiscoveryRunId) {
    ...
    if ($ResumeDiscoveryRunId) {
        Invoke-GhaWorkflow $DiscoveryWorkflow @{ resume_artifact_run_id = $ResumeDiscoveryRunId } -ContinueOnDiscoveryArtifact
...
if (-not $SkipBackfill) {
    Invoke-GhaWorkflow "PL niedziela backfill"
}

Invoke-GhaWorkflow "Sync wyniki Google Drive PL" @{ artifact_name = "pl-materialy-wyniki-thu" }
Invoke-GhaWorkflow "PL poniedzialek prep"
Invoke-GhaWorkflow "PL poniedzialek send" $sendFields
Invoke-GhaWorkflow "PL wtorek send" $sendFields
